...
static void UI_Update(const char *name) {
	int	val = trap_Cvar_VariableValue(name);
 	...
 	if (Q_stricmp(name, "ui_SetName") == 0) {
		...
		float sacc_rate = trap_Cvar_VariableValue("sacc_rate");
		if (sacc_rate >= 7500) {
			trap_Cvar_Set("sacc_maxpackets", "10");
			trap_Cvar_Set("sacc_packetdup", "1");
		} else if (sacc_rate >= 12500) {
			trap_Cvar_Set("sacc_maxpackets", "30");
			trap_Cvar_Set("sacc_packetdup", "2");		// favor less prediction errors when there's packet loss
		} else {
			trap_Cvar_Set("sacc_maxpackets", "60");
			trap_Cvar_Set("sacc_packetdup", "1");		// favor lower bandwidth
		}
