...
const char *FS_LoadedPakPureChecksums( void ) {
	...
	for(i = 0; i < 1; i++) {
		...
		if(!found) {
			Q_strcat(info, sizeof(info), va("pak%i", pak_purechecksums[i]));
		}
	}
...
Called on each restart, calculates the pure sums for non-exsistent base paks
================
*/
